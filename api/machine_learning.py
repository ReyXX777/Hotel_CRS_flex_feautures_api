...
def recommend_rooms(user_id):
    ...
    # Get all room types the user has booked
    ...
    booked_room_types = [Room.query.get(room_id).room_type for room_id in booked_room_ids]

    # Machine learning recommendation logic using text similarity
    vectorizer = TfidfVectorizer()
    available_rooms = Room.query.filter(Room.available.is_(True)).all()
    available_room_types = [room.room_type for room in available_rooms]

    # Combine room types (user's and available ones) and fit TF-IDF model
    all_room_types = booked_room_types + available_room_types
    vectors = vectorizer.fit_transform(all_room_types)

    # Apply nearest neighbors to find the closest room types
    knn = NearestNeighbors(n_neighbors=3)
    knn.fit(vectors[:len(booked_room_types)])

    # Recommend rooms by comparing user's booked rooms and available rooms
    recommended_indices = knn.kneighbors(vectors[len(booked_room_types):], return_distance=False)
    recommended_rooms = [available_rooms[i[0]] for i in recommended_indices]
